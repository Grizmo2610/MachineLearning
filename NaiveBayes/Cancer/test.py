...
# Preprocessing
data_path = "C:/HoangTu/Programing/University/MAT3533/NaiveBayes/Cancer/datacum.txt"
with open(data_path, 'r') as f:
    data = f.readlines()

data = [list(map(int, x.strip().split(","))) for x in data if (x[0] != '#' and len(x) > 1)]

# Remove index
data = [x[1:len(x) - 1] for x in data]

data = np.array(data)

# Split data into classes
X_benign = data[data[:, 0] == 2][:, 2:]
X_malignant = data[data[:, 0] == 4][:, 2:]
